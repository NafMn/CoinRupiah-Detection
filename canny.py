...
class CoinDetector:
    ...
    def display_result(self, canny_image):
        ...
        combined_image = np.hstack((canny_resized_bgr, resized_image))

        cv2.imshow('Menggunakan Canny | Mohamad Nafis', combined_image)
        cv2.waitKey(0)
    ...
    def process_image(self):
        ...
        canny_image, circles = self.detect_circles(preprocessed_image)
        self.process_detected_coins(circles)
        self.draw_contours_on_image()
        self.draw_text_on_image()
        self.display_result(canny_image) 
